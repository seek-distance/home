    ...
    var rootUrl="http://mdnwxapi.meitaitai.com.cn/";
    var rootUrl="http://mdnbeauty.demo.com/index.php/";
...
function (document,$){
    ...
    function event(e){
        ...
        if(!e.def){
            ...
            if(e.tel && localStorage.api=='true'){
                api.call({
                    type: 'tel_prompt',
                    number:e.url.slice(4),
                });
            }else{
    ...
    function end(e){
        ...
        if(time3 - time1 <300){
            ...
            if((!xy2) || Math.abs(xy3[0]-xy1[0])<3 && Math.abs(xy3[1]-xy1[1])<3){
                ...
                if(android && focus_el.indexOf(e.target)!=-1){return}
...
function(mui,window,$){
    ...
    // 获取位置
    window.getPosition=function(fn,err){
    ...
    mui.initPull=function (url,inputdata,pull,fn){
        ...
        inputdata.count=inputdata.cont
...
if(localStorage.api=='true'){
    ...
    script=script[script.length-1].getAttribute('src')

    document.write('<script src="'+script.slice(0,script.length-4)+'apiready.js"></script>')
}
else{
    setTimeout(function(){
